...
public abstract class DestinationFilter implements BooleanExpression {
    ...
    public boolean matches(MessageEvaluationContext message) throws JMSException {
        try {
            ...
            return matches((HedwigDestination) message.getMessage().getJMSDestination());
        } catch (IOException e) {
            throw JMSUtils.createJMSException(e.getMessage(), e);
        }
    }

    public abstract boolean matches(HedwigDestination destination);

    public static DestinationFilter parseFilter(HedwigDestination destination) {
